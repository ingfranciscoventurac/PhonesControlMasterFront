...
@Component({
  ...
  imports: [MatFormFieldModule, MatInputModule, FormsModule, MatButtonModule, MatFormFieldModule,
    ...
    MatDialogActions,
    MatDialogClose,],
  templateUrl: './addplaylistmodal.component.html',
  styleUrls: ['./addplaylistmodal.component.scss'],
...
export class AddplaylistmodalComponent {
  ...
  playOnDevices() {
    if (this.data.devicesList.length == 0 || this.spotifyUrl.length == 0) {
      this.messageService.showMessage("Necesitas agregar el/los enlace(s) de playlist que deseas reproducir.", "error");
      return;
    }
    this.devicesService.playPlaylist(this.data.devicesList, this.spotifyUrls).then(() => {
  ...
  onSpotifyUrlDrop(event: DragEvent) {
    ...
    if (!match) {
      console.warn('Invalid Spotify Playlist URL:', url);
      return;
    }
